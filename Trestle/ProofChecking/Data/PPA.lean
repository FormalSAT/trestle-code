...
def litValue? (τ : PPA) (l : ILit) : Option Bool :=
  τ.varValue? (l.toIVar) |>.map (xor !(l.polarity))
...
def litValue?_8 (τ : PPA) (l : ILit) : UInt8 :=
  let res := τ.varValue?_8 (l.toIVar)
  if l.polarity then
    res
  else
    PPA.negate res
...
def new (maxVar : Nat) : PPA where
  ...
  maxGen := 0
  le_maxGen i h := by rw [← Array.mem_data] at h; simp_all
...
def setLitUntil (τ : PPA) (l : ILit) (gen : Nat) : PPA :=
  τ.setVarUntil l.toIVar l.polarity gen
...
def setLitFor (τ : PPA) (l : ILit) (extraBumps : Nat) : PPA :=
  τ.setVarFor l.toIVar l.polarity extraBumps
...
theorem setNegatedClause_cons (τ : PPA) (l : ILit) (ls : List ILit) :
    setNegatedClause τ { data := l :: ls } = setNegatedClause (τ.setLit (-l)) { data := ls } := by
  simp only [setNegatedClause, Array.foldl_cons]

theorem setNegatedClause_loop_drop (τ : PPA) {C : List ILit} {i j : Nat} :
    j = C.length - i →
      setNegatedClause_loop.loop { data := C } τ i = setNegatedClause τ { data := C.drop i } := by
  ...
  | succ j ih =>
    ...
    rw [setNegatedClause_loop.loop]
    simp only [Array.size_mk, hi, ↓reduceDIte, Array.get_eq_getElem]
    rw [ih]
    match h_drop : C.drop i with
    | [] =>
      have := List.drop_eq_nil_iff_le.mp h_drop
      omega
    | (l :: ls) =>
      ...
      · simp at h_drop
        ...
        have : ({ data := C } : Array ILit)[i] = C[i] := rfl
        simp only [this, h_Ci]
      · exact hi
...
-- CC: TODO: Proof that they are equivalent?
