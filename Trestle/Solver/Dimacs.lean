...
def formatVar : IVar → String := IVar.toString
...
def formatAssn (a : Assn) : String :=
  (a.foldl (fun s l => s ++ " " ++ formatLit l) "v") ++ " 0"

def printRes [Monad m] [MonadExcept ε m] [Inhabited ε] (print : String → m Unit) : Solver.SolverResult → m Unit
| .sat assn => do
  print "s SATISFIABLE"
...
def parseAssnLine (maxVar : Nat) (assn : Assn) (s : String) : Except String Assn := do
  let nums := s.splitOn " "
  let mut assn : Assn := assn
...
def parseAssnLines (maxVar : Nat) (s : String) : Except String (List Assn) := do
  let lines :=
      s.splitOn "\n"
    |>.filter (fun line => !line.startsWith "c" && line.any (!·.isWhitespace))
...
/--
  Parses the assignment from the `v` line of a SAT solver's output.

  While there is no specification that requires that the literals listed in
  the (partial) assignment be in order, we will (for now) assume that CaDiCaL,
  etc. are nice to us and point the literals in increasing order.
-/
def parseVLines (maxVar : Nat) (assn : Assn) (s : String) : Except String Assn := do
...
def parseResult (maxVar : Nat) (s : String) : Except String Solver.SolverResult := do
  let lines :=
    s.splitOn "\n"
    |>.filter (fun line => !line.startsWith "c" && line.any (!·.isWhitespace))
  ...
  | "s SATISFIABLE" =>
    let assn ←
      rest.foldlM (fun assn line => parseVLines maxVar assn line) #[]
    return .sat assn
